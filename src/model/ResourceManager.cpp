...
double &ResourceManager::requestSlot(const ResourceSlot &rs) {
    auto &resource_slot = resource_vps_.at(rs.resource).at(rs.slot);
    if (rs.slot == SlotType::BASE)
        resource_slot.push_back(0);
    else
        resource_slot.push_back(1);
    ...
    for (int i{}; i < resource_values_.size(); ++i) {
        ...
        resource_values_[i] +=
            delta * std::pow(std::reduce(base_vals.begin(), base_vals.end()) *
                                 std::accumulate(mult_vals.begin(), mult_vals.end(), 1.0,
                                                 std::multiplies<double>()),
                             std::accumulate(exp_vals.begin(), exp_vals.end(), 1.0,
                                             std::multiplies<double>()));
    }
}
} // namespace DI